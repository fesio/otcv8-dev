...
if type(storage.manaTrain) ~= "table" then
  storage.manaTrain = {on=false, title="MP%", text="exura gran san", min=80, max=100}
end
...
local iconImageID = 3048
addIcon("MIGHT-RING & STONE SKIN", 
{item=iconImageID, movable=true,
text = "Might & Stone"}, 

macro(190, "MIGHT-RING & STONE SKIN", function(icon)
  -- For Might Ring
  local mightRing = 3048
  if getFinger() == nil or getFinger():getId() ~= mightRing then
    g_game.equipItemId(mightRing)
  end

  -- For Stone Skin Amulet
  local stoneSkinAmulet = 3081
  if getNeck() == nil or getNeck():getId() ~= stoneSkinAmulet then
    g_game.equipItemId(stoneSkinAmulet)
  end
